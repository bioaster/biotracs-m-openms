classdef IDPosteriorErrorProbabilityTests < matlab.unittest.TestCase
    
    properties (TestParameter)
    end
    
    properties
        workingDir = [biotracs.core.env.Env.workingDir, '/biotracs/openms/IDPosteriorErrorProbabilityTests'];
    end

    methods (Test)
        
        function testWithLocalFiles(testCase)
            dataFile1 = biotracs.data.model.DataFile( [pwd, '/../../../testdata/XtandemidXML/Protein_trypsin_996.idXML'] );
            dataFile2 = biotracs.data.model.DataFile( [pwd, '/../../../testdata/XtandemidXML/Protein_trypsin_994.idXML'] );
            ds = biotracs.data.model.DataFileSet();
            ds.add(dataFile1);
            ds.add(dataFile2);

            process = biotracs.openms.model.IDPosteriorErrorProbability();
            c = process.getConfig();
