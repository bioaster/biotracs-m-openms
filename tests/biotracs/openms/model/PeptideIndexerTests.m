classdef PeptideIndexerTests < matlab.unittest.TestCase
    
    properties (TestParameter)
    end
    
    properties
        workingDir = [biotracs.core.env.Env.workingDir, '/biotracs/openms/PeptideIndexerTests'];
    end
    ...
    methods (Test)
        ...
        function testWithLocalFiles(testCase)
            ...
            fastaFile= biotracs.data.model.DataFile( [pwd, '/../../../testdata/fasta/small_decoy_fasta.fasta'] );
            ds = biotracs.data.model.DataFileSet();
            ds.add(dataFile1);
            ds.add(dataFile2);

            process = biotracs.openms.model.PeptideIndexer();
            c = process.getConfig();
            process.setInputPortData('DataFileSet', ds);
            process.setInputPortData('DatabaseFile', fastaFile);
            c.updateParamValue('WorkingDirectory', testCase.workingDir);
            c.updateParamValue('MissingDecoyAction', 'warn');
